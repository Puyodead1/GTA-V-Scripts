#region Local Var
	int scaleformMovieOpeningCredits = 0;
	int timeout = 0;
	BOOL hasLogoShown = 0;
#endregion
...
void main() // Position - 0x0
{
	...
	scaleformMovieOpeningCredits = GRAPHICS::REQUEST_SCALEFORM_MOVIE("OPENING_CREDITS");

	while (!GRAPHICS::HAS_SCALEFORM_MOVIE_LOADED(scaleformMovieOpeningCredits))
	{
		func_7();
		SYSTEM::WAIT(0);
	}
	...
	ShowOpeningCreditsLogo("TITLE", 0f, 1f, 1.5f, 1f, 0f, 1.5f, 15f);
	timeout = MISC::GET_GAME_TIMER() + 8000 + SYSTEM::ROUND(2f * 1000f);
	...
	while (!CUTSCENE::IS_CUTSCENE_PLAYING())
	{
		...
		{
			GRAPHICS::DRAW_SCALEFORM_MOVIE_FULLSCREEN(scaleformMovieOpeningCredits, 255, 255, 255, 255, 0);
		
			if (!hasLogoShown)
			{
				if (MISC::GET_GAME_TIMER() > timeout)
				{
					HideOpeningCreditsLogo("TITLE");
					hasLogoShown = true;
					timeout = MISC::GET_GAME_TIMER() + SYSTEM::ROUND(2f * 1000f);
				}
			}
			else if (MISC::GET_GAME_TIMER() > timeout)
			{
...
void HideOpeningCreditsLogo(char* text) // Position - 0x1D3
{
	GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(scaleformMovieOpeningCredits, "HIDE_LOGO");
...
void ShowOpeningCreditsLogo(char* text, float fParam1, float fParam2, float fParam3, float fParam4, float fParam5, float fParam6, float fParam7) // Position - 0x1FA
{
	GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(scaleformMovieOpeningCredits, "SHOW_LOGO");
	GRAPHICS::BEGIN_TEXT_COMMAND_SCALEFORM_STRING("STRING");
	HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(text);
	GRAPHICS::END_TEXT_COMMAND_SCALEFORM_STRING();
...
void DisableReplayRecording() // Position - 0x268
{
...
void CleanupAndTerminate() // Position - 0x286
{
	MISC::SET_BIT(&(G_MissionStats.f_10019.f_25), 1);

	if (scaleformMovieOpeningCredits != 0)
		GRAPHICS::SET_SCALEFORM_MOVIE_AS_NO_LONGER_NEEDED(&scaleformMovieOpeningCredits);
